...
from tad.protocols.protocol_state_machine import message_requires_reply
from tad.protocols.protocol_timing import INVALID_PROTOCOL_BAN_SECONDS, API_EXCEPTION_BAN_SECONDS
...
from tad.util.ssl import verify_ssl_certs_and_keys
...
class TadServer:
    ...
    async def start_server(self, on_connect: Callable = None):
        ...
        if authenticate:
            ...
            ssl_context = ssl_context_for_server(
                self.tad_ca_crt_path, self.tad_ca_key_path, self.p2p_crt_path, self.p2p_key_path, log=self.log
            )
    ...
    def get_connections(self, node_type: Optional[NodeType] = None) -> List[WSTadConnection]:
        result = []
        for _, connection in self.all_connections.items():
