...
from tad.util.keychain import KeyringIsLocked, bytes_to_mnemonic, generate_mnemonic
...
class WalletRpcApi:
    ...
    async def _check_key_used_for_rewards(
        ...
        farmer_target = config["farmer"].get("tad_target_address")
        pool_target = config["pool"].get("tad_target_address")
        found_farmer = False
        found_pool = False
