...
from tad.ssl.create_ssl import (
    ensure_ssl_dirs,
...
from tad.util.config import (
    create_default_tad_config,
    initial_config_file,
    load_config,
    save_config,
...
from tad.wallet.derive_keys import master_sk_to_pool_sk, master_sk_to_wallet_sk
...
def check_keys(new_root: Path, keychain: Optional[Keychain] = None) -> None:
    ...
    if len(all_sks) == 0:
        print("No keys are present in the keychain. Generate them with 'tad keys generate'")
        return None
    ...
    for i in range(number_of_ph_to_search):
        ...
        for sk, _ in all_sks:
            ...
            if all_targets[-1] == config["farmer"].get("tad_target_address"):
                stop_searching_for_farmer = True
            if all_targets[-1] == config["pool"].get("tad_target_address"):
                stop_searching_for_pool = True

    # Set the destinations, if necessary
    updated_target: bool = False
    if "tad_target_address" not in config["farmer"]:
        print(
            f"Setting the tad destination for the farmer reward (1/8 plus fees, solo and pooling) to {all_targets[0]}"
        )
        config["farmer"]["tad_target_address"] = all_targets[0]
        updated_target = True
    elif config["farmer"]["tad_target_address"] not in all_targets:
        print(
            f"WARNING: using a farmer address which we don't have the private"
            f" keys for. We searched the first {number_of_ph_to_search} addresses. Consider overriding "
            f"{config['farmer']['tad_target_address']} with {all_targets[0]}"
        )

    if "pool" not in config:
        config["pool"] = {}
    if "tad_target_address" not in config["pool"]:
        print(f"Setting the tad destination address for pool reward (7/8 for solo only) to {all_targets[0]}")
        config["pool"]["tad_target_address"] = all_targets[0]
        updated_target = True
    elif config["pool"]["tad_target_address"] not in all_targets:
        print(
            f"WARNING: using a pool address which we don't have the private"
            f" keys for. We searched the first {number_of_ph_to_search} addresses. Consider overriding "
            f"{config['pool']['tad_target_address']} with {all_targets[0]}"
        )
    if updated_target:
        print(
...
def create_all_ssl(root_path: Path):
    ...
    tad_ca_crt, tad_ca_key = get_tad_ca_crt_key()
    generate_ssl_for_nodes(ssl_dir, tad_ca_crt, tad_ca_key, False, overwrite=False)
...
def init(create_certs: Optional[Path], root_path: Path, fix_ssl_permissions: bool = False):
    if create_certs is not None:
        if root_path.exists():
            ...
            if tad_init(root_path, fix_ssl_permissions=fix_ssl_permissions) == 0 and root_path.exists():
                return init(create_certs, root_path, fix_ssl_permissions)

            print(f"** {root_path} was not created. Exiting **")
            return -1
    else:
        return tad_init(root_path, fix_ssl_permissions=fix_ssl_permissions)


def tad_version_number() -> Tuple[str, str, str, str]:
...
def tad_init(root_path: Path, *, should_check_keys: bool = True, fix_ssl_permissions: bool = False):
    """
    ...
    """
    if os.environ.get("TAD_ROOT", None) is not None:
        print(
            f"warning, your TAD_ROOT is set to {os.environ['TAD_ROOT']}. "
            f"Please unset the environment variable and run tad init again\n"
            f"or manually migrate config.yaml"
        )

    print(f"Tad directory {root_path}")
    if root_path.is_dir() and Path(root_path / "config" / "config.yaml").exists():
        # This is reached if TAD_ROOT is set, or if user has run tad init twice
        # before a new update.
    ...
    create_default_tad_config(root_path)
    create_all_ssl(root_path)
    if fix_ssl_permissions:
        fix_ssl(root_path)
    if should_check_keys:
