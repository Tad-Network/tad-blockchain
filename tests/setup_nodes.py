...
from tad.util.keychain import bytes_to_mnemonic
...
async def setup_farmer(
    ...
    config["tad_target_address"] = encode_puzzle_hash(b_tools.farmer_ph, "tad")
    config["pool_public_keys"] = [bytes(pk).hex() for pk in b_tools.pool_pubkeys]
    config["port"] = port
    config_pool["tad_target_address"] = encode_puzzle_hash(b_tools.pool_ph, "tad")
